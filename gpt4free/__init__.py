...
class Completion:
    ...
    def __useless_service(prompt: str, **kwargs) -> str:
        return gpt4free.usesless.Completion.create(prompt = prompt, **kwargs)

    @staticmethod
    def __you_service(prompt: str, **kwargs) -> str:
        return gpt4free.you.Completion.create(prompt, **kwargs).text

    @staticmethod
    def __poe_service(prompt: str, **kwargs) -> str:
        return gpt4free.quora.Completion.create(prompt=prompt, **kwargs).text

    @staticmethod
    def __fore_front_service(prompt: str, **kwargs) -> str:
        return gpt4free.forefront.Completion.create(prompt=prompt, **kwargs).text

    @staticmethod
    def __theb_service(prompt: str, **kwargs):
        return ''.join(gpt4free.theb.Completion.create(prompt=prompt))

    @staticmethod
    def __cocalc_service(prompt: str, **kwargs):
        return gpt4free.cocalc.Completion.create(prompt, cookie_input=kwargs.get('cookie_input', '')).text
